// app/konto/page.tsx – Schutz-Dashboard (Sidebar-Layout) + ehrliche Conversion-Nudges.
...
export default async function KontoPage() {
  ...
  const ConversionCard = (
    <section id="fall-begleitung" className="scroll-mt-6 overflow-hidden rounded-4xl border border-mint/25 bg-gradient-to-br from-mint/12 via-night-surface to-night-inset p-6 bezel-soft sm:p-7">
      ...
      <p className="mt-5 leading-relaxed text-slate-300">
        ...
        Verpasst du sie, drohen Mahnbescheid und zusätzliche Kosten. Mit der Fall-Begleitung erinnern
        wir dich <strong className="font-semibold text-white">automatisch</strong> und sagen dir bei jedem
        neuen Brief, was zu tun ist.
      </p>

      <ul className="mt-4 space-y-2">
        {BENEFITS.map((b) => (
          <li key={b} className="flex gap-2.5 text-sm text-slate-300"><Check />{b}</li>
        ))}
      </ul>
  ...
  return (
    <DashboardShell email={user.email} premiumActive={isPremium} premiumLabel={premiumLabel}>
      ...
      <p className="mt-2 text-slate-400">
        {hasCases
          ? "Der Stand deiner Fälle und anstehende Fristen – ruhig und übersichtlich."
        ...
        <>
          <section className="mt-8">
            <div className="flex flex-wrap items-baseline justify-between gap-2">
              ...
              <a href="/" className="text-sm font-semibold text-mint-light hover:underline">Neue Forderung prüfen</a>
            </div>
          ...
          {isPremium ? (
            <section id="fall-begleitung" className="mt-8 scroll-mt-6 rounded-4xl border border-mint/25 bg-night-surface/60 p-6 bezel-soft">
              <p className="flex items-center gap-2 text-[11px] font-bold uppercase tracking-[0.18em] text-mint-light"><Check /> Fall-Begleitung aktiv</p>
            ...
            <div className="mt-8">{ConversionCard}</div>
          )}
        </>
      ) : (
        <>
          ...
          {!isPremium ? (
            <section id="fall-begleitung" className="mt-6 scroll-mt-6 rounded-4xl border border-white/10 bg-night-surface/60 p-6 bezel-soft">
              <div className="flex flex-wrap items-baseline justify-between gap-2">
